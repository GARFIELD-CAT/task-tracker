...
def upgrade() -> None:
    ...
    for i in range(1, 101):
        ...
        tasks.append(
            f"('Test task {i}', 'Description for task {i}', {assignee_id}, '{status}', {created_at}, {updated_at}, {closed_at}, {started_work_at})"
        )
...
def downgrade() -> None:
    ...
    op.execute(
        "DELETE FROM users WHERE email IN ('user1@example.com', 'user2@example.com', 'user3@example.com', 'admin@example.com')"
    )
